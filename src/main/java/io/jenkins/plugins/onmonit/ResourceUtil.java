...
public class ResourceUtil {
    ...
    public static String getOtelFilename(String os, boolean isAmd64) {
        if ("win".equals(os)) {
            return "otelcol-contrib_0.135.0_win_amd64.exe";
        } else if ("darwin".equals(os)) {
            return "otelcol-contrib_0.135.0_darwin_" + (isAmd64 ? "amd64" : "arm64");
        } else {
            return "otelcol-contrib_0.135.0_linux_" + (isAmd64 ? "amd64" : "arm64");
        }
    }
    ...
    public static String getNodeExporterFilename(String os, boolean isAmd64) {
        if ("win".equals(os)) {
            ...
            return "node_exporter-1.9.1.darwin-" + (isAmd64 ? "amd64" : "arm64");
        } else {
            return "node_exporter-1.9.1.linux-" + (isAmd64 ? "amd64" : "arm64");
        }
    }
